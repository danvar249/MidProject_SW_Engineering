		...
		machineDataMap.forEach((machine, data) -> {
			...
			if (counter > 0) {
				...
				if (LocalDate.now().getMonthValue() < 10) {
					...
					month = new Integer(LocalDate.now().getMonthValue()).toString();
				}
				SCCP restockMachine = new SCCP();
				restockMachine.setRequestType(ServerClientRequestTypes.ADD);
				restockMachine.setMessageSent(new Object[] {"restock_in_machine", false, new Object[] {"(\"" + 
						LocalDate.now().toString() + "\", " + machine.getMachineId() + ", " + counter 
						+ ", \"" + month + "-" + LocalDate.now().getYear() + "\""}});
				ClientUI.clientController.accept(restockMachine);
			}
		});
